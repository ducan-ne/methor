...
function addRoute(router, path) {
  ...
  if ((0, _util.isObject)(router) || _isFunction) {
    var callbacks = _isFunction ? [router] : (0, _util.isArray)(router.handler) ? router.handler : [router.handler];
    if (callbacks.length == 0) {
      throw new TypeError('argument handler is required');
    }
    var methods = (router.method || 'get').split(',').map(function toLowerCase(method) {
    ...
    try {
      for (var _iterator = methods[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (!this[_method])
          ...
          throw new Error('Not support method ' + _method + ' !!!');
        this[_method].apply(this, [router.path].concat(_toConsumableArray(callbacks.map(function (callback) {
          return function () {
