...
function Init() {
	...
	try {
		for (var _iterator = _methods2.default.concat('all')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			var k = _step.value;
	...
	this.proxy = new Proxy(this, {
		get(target, name, _) {
			...
			if (name in server) {
				if ((0, _util.isFunction)(server[name])) {
					return function () {
	...
	var beforeEnter = [this.opts.beforeEnter].concat(_toConsumableArray(this._beforeEnter)).filter(_util.identity);
