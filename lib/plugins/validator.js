  ...
  return function install() {
    ...
    this.beforeEnter(function (req, res, next) {
      ...
      try {
        ...
        } else if (method.validate && method.validate.__type && method.validate.__type == 'payload') {
          params = typeof opts.payload === 'function' ? opts.payload(req, res) : JSON.parse(req.query.payload);
          isPayload = true;
        } else {
          params = req.query;
          isPayload = false;
      ...
      if (isPayload) {
        var isPayloadBody = Object.keys(req.body).length > 0;
